...
- The choice is stored on the grant and applied on every request through the same scope checker as API keys. It can only narrow access: the server drops collections, globals, actions and tools outside the site's `access` level and the plugin's exposed collections. Search, reference and upload tools work only when every collection and global is selected. The connections page shows each connection's access, for example "Read only · 3 of 12 collections · all globals · 7 of 9 tools". To change it, disconnect and connect again.
